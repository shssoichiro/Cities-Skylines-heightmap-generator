...
const sharpenKernel = [
  ...
  [-0.01563, -0.0625, -0.09375, -0.0625, -0.01563],
  [-0.02344, -0.09375, +1.8598, -0.09375, -0.02344],
  [-0.01563, -0.0625, -0.09375, -0.0625, -0.01563],
  [-0.00391, -0.01563, -0.02344, -0.01563, -0.00391],
];
...
mapboxgl.accessToken =
  "pk.eyJ1IjoiYmVydGRldm4iLCJhIjoiY2t2dXF1ZGhyMHlteTJ2bzJjZzE3M24xOCJ9.J5skknTRyh-6RoDWD4kw2w";
...
function getExtent(lng, lat, size = vmapSize) {
  ...
  let topleft = turf.destination(point, dist, -45, { units: "kilometers" })
    .geometry.coordinates;
  let bottomright = turf.destination(point, dist, 135, { units: "kilometers" })
    .geometry.coordinates;
...
function getGrid(lng, lat, size) {
  ...
  return turf.squareGrid(
    [
      extent.topleft[0],
      extent.topleft[1],
      extent.bottomright[0],
      extent.bottomright[1],
    ],
    tileSize,
    { units: "kilometers" }
  );
...
function loadSettings() {
  ...
  document.getElementById("waterDepth").value =
    parseInt(stored.waterDepth) || defaultWaterdepth;
  document.getElementById("tiltHeight").value =
    parseInt(stored.tiltHeight) || parseInt(stored.waterDepth / 2);
  ...
  document.getElementById("blurPasses").value =
    parseInt(stored.blurPasses) || 7;
  document.getElementById("blurPostPasses").value =
    parseInt(stored.blurPostPasses) || 3;
...
function togglePanel(index) {
  ...
  panels[index].setAttribute(
    "class",
    isOpens[index] ? "panel slide-out" : "panel slide-in"
  );
  icons[index].setAttribute(
    "class",
    isOpens[index] ? iconClass[index] : "icon fas fa-angle-left"
  );
...
function getHeightmap(mode = 0, callback) {
  ...
  let topDistance = turf.distance(
    turf.point([tileLng, tileLat]),
    turf.point([tileLng, extent.topleft[1]]),
    { units: "kilometers" }
  );
  let leftDistance = turf.distance(
    turf.point([tileLng, tileLat]),
    turf.point([extent.topleft[0], tileLat]),
    { units: "kilometers" }
  );
  ...
  let timer = window.setInterval(function () {
    ...
    if (isDownloadComplete(tiles, vTiles)) {
      ...
      let watermap = sanatizeWatermap(
        toWatermap(vTiles, heightmap.length),
        xOffset,
        yOffset
      );
      ...
      if (typeof vTiles[ty][tx] !== "boolean") {
        if (vTiles[ty][tx].layers.water) {
          ...
          for (let i = 0; i < geo.length; i++) {
            ctx.moveTo(
              Math.round(geo[i][0].x * coef + (tx * length) / tileCnt),
              Math.round(geo[i][0].y * coef + (ty * length) / tileCnt)
            );
            for (let j = 1; j < geo[i].length; j++) {
              ctx.lineTo(
                Math.round(geo[i][j].x * coef + (tx * length) / tileCnt),
                Math.round(geo[i][j].y * coef + (ty * length) / tileCnt)
              );
            }
          }
        }
...
function tiltMap(map, gravityCenter, waterDepth) {
    ...
    for (let x = 0; x < maxX; x++) {
      ...
      switch (gravityCenter) {
        ...
        case 9:
          ...
          relDistance =
            Math.sqrt(
              Math.pow(gravityPoint.x - x, 2) + Math.pow(gravityPoint.y - y, 2)
            ) / maxY;
          break;
        case 10:
        case 12:
  ...
  if (gravityCenter == 0) {
    ...
    console.log(
      `tilted map in direction ${gravityCenter} with ${waterDepth} m`
    );
  }
  return tiltedMap;
}
...
function levelMap(map, min, max, style) {
  ...
  for (let i = 0; i < interpolatedCurve.length; i++) {
    levels.push(
      Math.round((lastLevel + elevationStep * interpolatedCurve[i]) * 10) / 10
    );
  ...
  for (let y = 0; y < maxY; y++) {
    for (let x = 0; x < maxX; x++) {
      ...
      if (h - min > 0) {
        ...
        let idx = Math.min(
          interpolatedCurve.length - 1,
          Math.floor((h - min) / elevationStep)
        );
  ...
  for (let i = 0; i < hmSize - 1; i++) {
    for (let j = 0; j < hmSize - 1; j++) {
      ...
      heightmap[i][j] = Math.round(
        (1 - dx) * (1 - dy) * srcMap[y0][x0] +
          dx * (1 - dy) * srcMap[y0][x1] +
          (1 - dx) * dy * srcMap[y1][x0] +
          dx * dy * srcMap[y1][x1]
      );
    }
  }
...
function toCitiesmap(heightmap, watermap) {
  ...
  let waterDepth =
    Math.round((scope.waterDepth / parseFloat(scope.heightScale)) * 100 * 10) /
    10;
  ...
  workingmap = levelMap(
    workingmap,
    grid.minHeight + waterDepth,
    grid.maxHeight,
    scope.levelCorrection
  );
  ...
  for (let l = 0; l < passes; l++) {
    workingmap = filterMap(
      workingmap,
      0,
      plainsHeight + waterDepth,
      meanKernel
    );
  }
  ...
  tiltHeight =
    Math.round((tiltHeight / parseFloat(scope.heightScale)) * 100 * 10) / 10;
